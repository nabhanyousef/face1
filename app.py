...
if not os.path.exists(PATIENT_CSV):
    with open(PATIENT_CSV, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Patient ID', 'Patient Name', 'Height', 'Weight', 'Gender'])
...
def download_report(patient_id):
    ...
    # Save the CSV file to disk (optional, for debugging)
    csv_filename = f"report_{patient_id}.csv"
...
def doctor_page():
    ...
    if request.method == 'POST':
        ...
        pdf.cell(200, 10, txt=f"Review: {review}", ln=True)
        ...
        flash('Report saved as PDF successfully!', 'success')
        return redirect(url_for('doctor_page'))
